...
if __name__ == '__main__':
    ...
    for i in tqdm.tqdm(range(REPEAT), ncols = 70):
        py_data.append(timeit.timeit(stmt = 'from cliffwalk_v2 import py_cliffwalk\n' 'py_cliffwalk.qlearn()\n', number = NUMBER))
    ...
    naive_cy_data = []
    for i in tqdm.tqdm(range(REPEAT), ncols = 70):
        naive_cy_data.append(timeit.timeit(stmt = 'from cliffwalk_v2 import ncy_cliffwalk\n' 'ncy_cliffwalk.qlearn()', number = NUMBER))
    ...
    cy_data = []
    for i in tqdm.tqdm(range(REPEAT), ncols = 70):
        cy_data.append(timeit.timeit(stmt = 'from cliffwalk_v2 import cy_cliffwalk\n' 'cy_cliffwalk.qlearn()', number = NUMBER))
